...
A model that computes the probability of a sentence $P(w_1 \cdots w_i)$ (or $P(w_1^i)$) or the probability of an upcoming word $P(w_i|w_1 \cdots w_{i-1})$ (or $P(w_i|w_1^{i-1})$) is called a **language model**. 

By the chain rule of probability, we have 

$$
P(w_1^i) = P(w_1)P(w_2|w_1)P(w_3|w_1^2) \cdots P(w_i|w_1^{i-1}) = \prod_{j=1}^{i} P(w_j | w_1^{j-1}).
$$
...
which looks $n − 1$ words into the past. In particular, unigram model: $P(w_1^i) \approx \prod_i P(w_i)$; bigram model: $P(w_i|w_1^{i-1}) \approx P(w_i|w_{i-1})$.

By using n-gram approximation, we have

$$
P(w_1^i) = \prod_{j=1}^{i} P(w_j | w_1^{j-1}) \approx \prod_{j=1}^{i} P(w_j | w_{j-n+1}^{j-1}). 
$$
...
$$
\text{PP}(w_1^N) \approx \prod_{i=1}^N \hat{P}(w_i|w_{i-n+1}^{i-1})^{-1/N}.
$$
...
Denote $\text{Count}(\cdot)$ as $C(\cdot)$, and suppose there are $V$ words in the vocabulary and $N$ word tokens, then we have $P_{\text{add-1}}(w_i) = \frac{C(w_i)+1}{N+V}, C_{\text{add-1}}(w_i) = (C(w_i)+1) \frac{N}{N+V}, P_{\text{add-1}}(w_i|w_{i-1})=\frac{C(w_{i-1}w_i)+1}{C(w_{i-1})+V}$.

Instead of adding 1 to each count, we add a fractional count $k\ (0.5?, 0.05?, 0.01?)$, it follows that $P_{\text{add-k}}(w_n|w_{n-1})=\frac{C(w_{n-1}w_n)+k}{C(w_{n-1})+kV}$. This algorithm is called **add-k smoothing**. It turns out that it still doesn't work well for language modeling.

### Backoff and Interpolation

The intuition is that, if we are trying to compute $P(w_n|w_{n−2}w_{n−1})$ but we have no examples of a particular trigram $w_{n−2}w_{n−1}w_{n}$, we can instead estimate its probability by using the bigram probability $P(w_n|w_{n−1})$. Similarly, if we don't have counts to compute $P(w_n|w_{n−1})$, we can look to the unigram $P(w_n)$.
...
$$
\hat{P}(w_i|w_{i-2}w_{i-1}) = \lambda_1 P(w_i|w_{i-2}w_{i-1}) + \lambda_2 P(w_i|w_{i-1}) + \lambda_3 P(w_i), \text{ such that } \sum_{j=1}^3{\lambda_j}=1.
$$
...
$$
\hat{P}(w_n|w_{n-2}w_{n-1}) = \lambda_1(w_{n-2}^{n-1}) \cdot P(w_n|w_{n-2}w_{n-1}) + \lambda_2(w_{n-2}^{n-1}) \cdot P(w_n|w_{n-1}) + \lambda_3(w_{n-2}^{n-1}) \cdot P(w_n).
$$
...
$$
P_{\text{AD}}(w_i|w_{i-1}) = \frac{C(w_{i-1}w_i)-d}{\sum_v C(w_{i-1}v)} + \lambda(w_{i-1}) \cdot P(w_i).
$$
...
$$
P_{\text{KN}}(w_i|w_{i-1}) = \frac{\max(C(w_{i-1}w_i)-d, 0)}{C(w_{i-1})} + \lambda(w_{i-1}) \cdot P_{\text{Conti}}(w_i).
$$
...
The first term, $\frac{d}{\sum_u C(w_{i-1}u)}$ , is the normalized discount. The second term, $|\{u: C(w_{i−1}u) > 0\}|$, is the number of word types that can follow $w_{n−1}$ or, equivalently, the number of word types that we discounted.

The general recursive formulation is as follows:

$$
P_{\text{KN}}(w_i|w^{i-1}_{i-n+1}) = \frac{\max(C_{\text{KN}}(w^{i-1}_{i-n+1})-d, 0)}{\sum_v{C_{\text{KN}}(w^{i-1}_{i-n+1} v)}} + \lambda(w^{i-1}_{i-n+1}) \cdot P_{\text{KN}}(w_i|w^{i-1}_{i-n+2}).
$$
