...
We want to minimize $L(y_i, f(x_i)), i=1,...N$, where $f$ is the independent variable. According to gradient descent, at $m$-th step, $f_m = f_{m-1} - \eta_m \nabla \text{Loss}_m$, where $\eta_m$ is the step size and  $$ \nabla \text{Loss}_m = \frac{∂L(y_i,f_{m-1}(x_i))}{∂f_{m-1}(x_i)} $$ is the gradient.
...
$$
r_{im} = -\bigg[ \frac{∂L(y_i,f (x_i))}{∂f(x_i)} \bigg] _{f=f_{m-1}}.
$$

 (b) Fit a regression tree to the targets $r_{im}$'s to get terminal regions $R_{jm},j=1,2,..,J_m$.
