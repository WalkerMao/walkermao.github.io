...
where $g_{im} = \frac{∂L(y_i,f_{m-1}(x_i))}{∂f_{m-1}(x_i)}, h_{im} = \frac{∂L^2(y_i,f_{m-1}(x_i))}{∂f_{m-1}^2(x_i)}$. 
...
[2] $$f_m(x) = f_{m−1}(x) + \eta_m h_m(x)$$ where $\eta_m$ is called step-size or learning rate, usually set around $0.1$. 
