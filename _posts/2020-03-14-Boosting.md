...
<img src="https://static.packt-cdn.com/products/9781789953633/graphics/23c1ad86-604f-439f-9597-73c645e41a74.png" alt="AdaBoost" style="zoom: 80%;" />
...
In the algorithm AdaBoost.M1. The basis functions are the individual classifiers $G_m(x) ∈ \{−1, 1\}$.
...
Gradient boosting is based on gradient descent. We induce a weak regressor or classifier $h_m(x)$ (e.g. a tree $T(x;Θ_m)$) at the $m$-th iteration to fit the negative gradient, which means the predictions (a vector with dimension $n \times 1$) by weak model $h_m(x)$ are as close as possible to the negative gradient. 
...
As shown in my earlier blog [*Intro to Statistical Learning*](https://walkermao.github.io/ml/2020/02/28/Intro-to-ML.html), $$h_m(x_i)$$ should be $- \eta_m g_{im}$, where $\eta_m$ is the step size and  $$ g_{im} = \frac{∂L(y_i,f_{m-1}(x_i))}{∂f_{m-1}(x_i)} $$ is the gradient. 
...
We use **Taylor expansion** of second order to get the approximation of loss function. Recall Taylor expansion: $$f(x+\Delta x) \approx f(x)+f'(x)\Delta x + \frac{1}{2} f''(x)\Delta x^2$$. We do that for $$L(y_i, f_{m-1}(x)+h_m(x_i))$$, and we have
...
Taylor expansion of second order, regularization and shrinkage are the key reasons why XGBoost is better than GBM. 
...
[1] $$\begin{align*}
   h_m = \operatorname*{argmin}_{h_m} \text{Obj}^{(m)} = \operatorname*{argmin}_{h_m} \Big[g_ih_m(x_i) + \frac{1}{2}h_ih_m^2(x_i)\Big] + \Omega(h_{m}).
   \end{align*}$$

[2] $$\begin{align*}
   f_m(x) = f_{m−1}(x) + \eta_m \cdot h_m(x),
   \end{align*}$$ where $\eta_m$ is called step-size or shrinkage, usually set around $0.1$. 
