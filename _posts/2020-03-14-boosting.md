...
$$
f(x) = \sum_{m=1}^{M} \eta_m b(x; \mu_m),
$$

where $\eta_m , m = 1, 2, . . . , M$ are the expansion coefficients, and $b(x; \mu) ∈ \mathbb{R}$ are usually simple basis functions of the multivariate argument $x$, characterized by a set of parameters $\mu$. 
...
[2] For $m = 1$ to $M$:
 ...
 (a) Compute $$ (\eta_m, \mu_m) = \operatorname*{argmin}_{\eta,\mu} \sum_{i=1}^{N} L\big(y_i, f_{m−1}(x_i) + \eta b(x_i;\mu)\big)$$.

 (b) Set $f^{(m)}(x) = f^{(m−1)}(x) + \eta_mb(x;\mu_m)$.

[3] Output $f^{(M)}(x)$.

Suppose $\mu$ is of dimension $p_{\mu}$. In $m$-th step, we need to do optimization with $1+p_{\mu}$ parameters ($\eta_m$ is $1$ dimension and $\mu_m$ is $p_{\mu}$). At the end, $f^{(M)}(x)$ have $M(1+p_{\mu})$ parameters. That is much easier than we do optimization with $M(1+p_{\mu})$ parameters: $$(\eta, \mu) = \operatorname*{argmin}_{\eta,\mu} \sum_{i=1}^{N} L\big(y_i, f^{(M)}(x_i)\big)$$. 
...
1. Finding $\mu_j$ given $R_j$: Given the $R_j$, often $\hat{\mu}_j = \bar{y}_j$ , the mean of the $y_i$ falling in region $R_j$.
...
$$
\hat{Θ}_m = \operatorname*{argmin}_{Θ_m} \sum_{i=1}^{N} L \big(y_i, f^{(m-1)}(x_i) + T(x_i;\Theta_m) \big)
$$
...
$$
\sum_{i=1}^{N} L\Big(y_i,f^{(m-1)}(x_i)+h_m(x_i)\Big) \approx \sum_{i=1}^{N} \Big[L(y_i, f^{(m-1)}(x_i)) + g_{im} h_m(x_i) \Big].
$$

As shown in my [earlier blog](https://walkermao.github.io/intro-to-ml.html), $$h_m(x_i)$$ should be $- \eta_m g_{im}$, where $\eta_m$ is the step size and  $$ g_{im} = \frac{∂L(y_i,f^{(m-1)}(x_i))}{∂f^{(m-1)}(x_i)} $$ is the gradient. 

Thus, we have the iterative formula: 

$$
f^{(M)}(x_i) = f^{(m-1)}(x_i) - \eta_m g_{im}.
$$

If $L(y_i, f^{(m-1)}(x_i))=\frac{1}{2}[y_i-f^{(m-1)}(x_i)]^2$, then the gradient $\frac{∂L(y_i,f (x_i))}{∂f^{(m-1)}(x_i)}=f^{(m-1)}(x_i)-y_i$, which is the residual $r_{im}$. 
...
$$
 ...
 (c) For $j = 1,2,...,J_m$, compute $\mu_{jm}$. Now we get a new tree 

$$
h_m(x) = T(x; Θ_m) = \sum_{j=1}^{J_m} \mu_{jm}I(x \in R_{jm}).
$$

 (d) Compute step size $$ \eta_m = \operatorname*{argmin}_{\eta} \sum_{i=1}^N L\big(y_i,f^{(m-1)}(x_i)+ \eta h_m(x_i) \big) $$.

 (e) Update 

$$
f^{(m)}(x) = f^{(m-1)}(x) + \eta_m h_m(x),
$$

 where $h_m(x) = \sum_{j=1}^{J_m} \mu_{jm}I(x \in R_{jm})$ is a new tree ($m$-th tree).
