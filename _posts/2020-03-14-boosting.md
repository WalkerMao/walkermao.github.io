...
(c) Compute $$ \eta_m = \log \big( \frac{1 − \text{err}_m}{\text{err}_m} \big)$$;

(d) Set $$ w_i ← w_i · \exp\left[\eta_m \cdot \mathbf{1}(y_i \neq f_m (x_i))\right] $$.
...
1. Finding $$\mu_j$$ given $$R_j$$: Given the $$R_j$$, often $$\hat{\mu}_j = \frac{1}{\mid R_j\mid} \sum_{x_i \in R_j} y_i$$ , which is the mean of the $$y_i$$'s falling in region $$R_j$$.
