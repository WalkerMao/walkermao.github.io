...
The purpose of the forget gate is to delete information from the context that is no longer needed. The forget gate computes a weighted sum of the previous state's hidden layer $h_{t-1}$ and the current input $$x_t$$ and passes that through a sigmoid $$\sigma(\cdot)$$ to produce $f_t$.
...
A new memory $$\tilde{c}_t$$ is the consolidation of a new input word $$x_t$$ with the past hidden state $$h_{t-1}$$. The new memory is the actual information we need to extract from the previous hidden state and current inputs.

$$
\text{New memory cell: } \tilde{c}_t = \text{tanh}(W^{(c)}x_t + U^{(c)}h_{t-1}).
$$

### Input Gate

The purpose of the input gate is to select information from new memory. The input gate uses the input word $x_t$ and the past hidden state $$h_{t-1}$$ to produce $$i_t$$. 
