...
In the context layer of an LSTM, the new memory generation cell generates new memory, and the forget gate and input gate produce indicators to gate past memory (context) and new memory respectively. After that, the final memory generation cell generates final memory based on the indicators, past memory, and new memory. Finally, the output gate produces an indicator to gate final memory, and we generate new hidden state. 
